...
public class UpdateTest {
	...
	UpdatePaymentInitiationTransactionRequest updatePaymentInitiationTransactionRequest =	EnhancedRandom.random(UpdatePaymentInitiationTransactionRequest.class);
	
	ObjectMapper objectMapper = new ObjectMapper();
	ObjectWriter ow = objectMapper.writer().withDefaultPrettyPrinter();
	String requestJson = ow.writeValueAsString(updatePaymentInitiationTransactionRequest);
	
	HttpHeaders httpHeaders = getHeaders();
	
	when(updateImpl.execute(Mockito.any(UpdateP.class))).thenReturn(response);
	
	mvc.perform(MockMvcRequestBuilders.put("/PaymentInitiation/1/Update").headers(httpHeaders).accept(MediaType.APPLICATION_JSON)
